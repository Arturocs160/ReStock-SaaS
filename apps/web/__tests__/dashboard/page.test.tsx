...
describe('DashboardPage Component', () => {
  it('debe renderizar el título de la página y el perfil de la tienda', () => {
    ...
    expect(screen.getByRole('heading', { name: 'Inventario por Lotes' })).toBeInTheDocument();
    expect(screen.getByText('Administra tus productos y desglosa sus lotes para gestionar vencimientos e ingresos.')).toBeInTheDocument();
  ...
  it('debe buscar un producto por nombre o código de barras', () => {
    render(<DashboardPage />);
    const searchInput = screen.getByPlaceholderText(/Buscador por nombre o código/i);
    
    // Buscar 'Leche'
    fireEvent.change(searchInput, { target: { value: 'Leche' } });
    
    expect(screen.getByText('Leche Entera Lala 1L')).toBeInTheDocument();
    expect(screen.queryByText('Coca-Cola Original 600ml')).not.toBeInTheDocument();
  ...
  it('debe filtrar los productos por categoría', () => {
    render(<DashboardPage />);
    const selectCategory = screen.getByRole('combobox', { name: 'Filtrar por Categoría' });
