...
export function CartItemRow({ item }: CartItemRowProps) {
  const { incrementQuantity, decrementQuantity, updateQuantity, removeLote } = useCartStore();
  ...
  return (
    <div className="flex items-center justify-between p-3 border-b border-slate-100 last:border-0 hover:bg-slate-50 transition-colors rounded-lg">
      <div className="flex-1 min-w-0 pr-4">
        <h4 className="text-sm font-bold text-slate-800 truncate">{item.nombre_producto}</h4>
        <span className="text-[11px] text-slate-400 block mt-0.5 font-medium">Lote: {item.codigo_lote}</span>
        <span className="text-xs font-bold text-slate-800 block mt-1">${item.precio_unitario.toFixed(2)}</span>
      </div>
        ...
        <div className="flex items-center border border-slate-200 rounded-full bg-white shadow-sm overflow-hidden h-8">
          <button
            ...
            className={`w-8 h-full flex items-center justify-center ${
              isAtMin ? 'text-slate-300 cursor-not-allowed' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
            <Minus size={12} />
          </button>
