...
import { Tag, Plus, Search, Edit3, Trash2, RotateCcw, FolderKanban } from "lucide-react"
...
export default function CategoriasPanel({
  ...
  const [statusFilter, setStatusFilter] = useState<"todas" | "activas" | "eliminadas">("activas")
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Categorías de Productos</h2>
          <p className="text-sm text-gray-500">
            Administra las categorías de tu inventario. Las categorías eliminadas se pueden restaurar en cualquier momento.
          </p>
        </div>
        <button
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-900">
              {filteredCategorias.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-10 text-center text-gray-400">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="font-semibold text-sm">No se encontraron categorías</p>
                      <p className="text-xs">Prueba ajustando los filtros o crea una nueva categoría.</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filteredCategorias.map(cat => {
                  return (
                    <tr key={cat.id_categoria} className="hover:bg-gray-50/50 dark:hover:bg-gray-900/10 transition">
                      <td className="px-6 py-4 font-bold text-gray-900 dark:text-white">
                        <div className="flex items-center gap-3">
                          <span className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs border ${cat.activo ? "bg-green-50 text-[#00a365] border-green-100 dark:bg-green-950/20 dark:text-green-400 dark:border-green-900/50" : "bg-gray-100 text-gray-400 border-gray-200 dark:bg-gray-800 dark:text-gray-500 dark:border-gray-750"}`}>
                            <Tag className="w-3.5 h-3.5" />
                          </span>
                          <div>
                            <p className="text-sm font-bold">{cat.nombre}</p>
                            <p className="text-[10px] text-gray-400 font-mono font-normal">ID: {cat.id_categoria}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 max-w-xs truncate font-medium text-gray-600 dark:text-gray-400" title={cat.descripcion || ""}>
                        {cat.descripcion || <span className="italic text-gray-400 text-xs">Sin descripción</span>}
                      </td>
                      <td className="px-6 py-4 text-gray-400 whitespace-nowrap">
                        {new Date(cat.createdAt || new Date()).toLocaleDateString("es-MX", { year: 'numeric', month: 'short', day: 'numeric' })}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold ${cat.activo
                          ...
                          <span className={`w-1.5 h-1.5 rounded-full ${cat.activo ? "bg-[#00a365]" : "bg-red-500"}`}></span>
                      ...
                      <td className="px-6 py-4 text-right space-x-1 whitespace-nowrap">
                        {cat.activo ? (
                          <>
                            ...
                            <button
                              onClick={() => handleToggleCategoryActive(cat.id_categoria)}
