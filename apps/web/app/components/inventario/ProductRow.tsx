...
import { ProductoConStock, LoteInventario, getExpirationStatus } from "../../dashboard/inventario/page";
        ...
        <>
            ...
            <tr className="hover:bg-gray-50/50 dark:hover:bg-gray-900/20 transition-all">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${isLowStock
                        ...
                        <span className={`w-1.5 h-1.5 rounded-full ${isLowStock ? "bg-red-500" : "bg-[#00a365]"}`}></span>
            ...
            {isExpanded && (
                <tr>
                    <td colSpan={5} className="bg-gray-50/50 dark:bg-gray-900/10 px-8 py-4 border-b border-gray-100 dark:border-gray-900">
                        <div className="space-y-3">
                            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide">Desglose de lotes para este producto:</h4>
                            {producto.lotes.length > 0 ? (
                                    ...
                                    <table className="w-full min-w-[650px] text-left text-xs">
                                        ...
                                        <tbody className="divide-y divide-gray-100 dark:divide-gray-900">
                                            {producto.lotes.map(l => {
                                                const expStatus = getExpirationStatus(l.fecha_caducidad);
                                                return (
                                                    <tr key={l.id_lote} className="hover:bg-gray-50/30 dark:hover:bg-gray-900/5 transition">
                                                        ...
                                                        <td className="px-4 py-2.5 text-gray-500 whitespace-nowrap">{l.fecha_ingreso}</td>
                                                        <td className="px-4 py-2.5 text-gray-500 whitespace-nowrap">{l.fecha_caducidad || "N/A"}</td>
                                                        ...
                                                        <td className="px-4 py-2.5 whitespace-nowrap">
                                                            <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-bold ${expStatus.color}`}>
                                                                {expStatus.label}
                                                            </span>
                                                        </td>
                                ...
                                <div className="text-center py-4 bg-white dark:bg-[#0c0c0c] border border-dashed border-gray-200 dark:border-gray-800 rounded-xl">
                                    <p className="text-xs text-gray-400 italic">No hay lotes ingresados para este producto. Registra uno nuevo para darle stock.</p>
                                </div>
                            )}
                        </div>
                    </td>
