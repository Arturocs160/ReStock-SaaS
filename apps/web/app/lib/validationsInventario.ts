...
export const productoSchema = z.object({
    nombre: z.string()
        ...
        .regex(/^[a-zA-ZáéíóúÁÉÍÓÚñÑüÜ0-9\s-]+$/, "El nombre solo puede contener letras, números, espacios y guiones medios"),
    ...
    precio_actual: z.number({ message: "El precio debe ser un número" })
        .positive("El precio debe ser un número positivo"),
    stock_minimo_sugerido: z.number({ message: "El stock mínimo debe ser un número" })
...
export const createLoteSchema = z.object({
    codigo_lote: z.string()
        ...
        .regex(/^L-\d+$/, "El código del lote debe comenzar con 'L-' seguido únicamente de números (por ejemplo: L-689554)"),
    ...
    fecha_caducidad: z.string().nullable().optional().or(z.literal("")).transform(val => val === "" ? null : val),
}).refine((data) => {
...
export const editLoteSchema = z.object({
    codigo_lote: z.string()
        .min(1, "El código del lote es obligatorio")
        .regex(/^L-\d+$/, "El código del lote debe comenzar con 'L-' seguido únicamente de números (por ejemplo: L-689554)"),
    cantidad_actual: z.number({ message: "La cantidad debe ser un número" })
        .int("La cantidad debe ser un número entero")
