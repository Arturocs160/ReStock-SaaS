...
export async function createVentaController(req: Request, res: Response) {
  try {
    ...
    if (!id_negocio) {
      return res.status(401).json({ error: "No autorizado", message: "ID de negocio ausente en la sesión." });
    }

    if (!userid) {
      return res.status(401).json({ error: "No autorizado", message: "ID de usuario ausente en la sesión." });
    }
    ...
    res.status(500).json({ error: "Error interno del servidor", message: "No fue posible procesar la venta." });
  }
}
