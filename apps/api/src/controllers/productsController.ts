...
import { createProductService, deleteProductService, getAllProductsServiceByTenantId, getProductByBarCodeService, getProductsByIdService, getProductsPaginationService, updateProductService } from "../services/productsServices";
...
export async function createProductController(req: Request, res: Response) {
    try {
        const id_negocio = req.user?.id_negocio;
        const { codigo_barras, nombre, precio_actual, stock_minimo_sugerido } = req.body;
        const product = await createProductService({ id_negocio, codigo_barras, nombre, precio_actual, stock_minimo_sugerido });
...
export async function updateProductController(req: Request, res: Response) {
    try {
        ...
        const product = await updateProductService({ id_producto, id_negocio, codigo_barras, nombre, precio_actual, stock_minimo_sugerido });
        res.status(200).json(product);
    } catch (error: any) {
